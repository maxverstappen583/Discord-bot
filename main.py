# main.py
# Full-featured Discord bot (prefix ? + slash /). Keep secrets in environment variables.
# Features included (requested):
# - Owner/admin/pookie system (defaults include OWNER_ID + two extra admin IDs)
# - Blacklist, blocked words, triggers (exact-word)
# - AFK system
# - Snipe & Esnipe with navigation buttons
# - Daily cat posting at 11:00 local TZ + optional hourly cats
# - Cat command (uses TheCatAPI if CAT_API_KEY set, otherwise fallback)
# - Say (no pings) and say_admin (pings allowed)
# - Moderation: ban/kick/purge (prefix & slash variants), mute/unmute, give/remove role, temp roles
# - Warn system (store warns in JSON)
# - Logs saved to JSON and send to configured log channel
# - Showcommands interactive view (shows only commands user can use)
# - Askforcommand (DM owner + ping in log channel)
# - Restart via Render API (if RENDER_API_KEY + RENDER_SERVICE_ID set)
# - Debug & eval (owner/admin controls)
# - Persistence to bot_data.json
# - Flask keepalive (for Render / UptimeRobot)
#
# IMPORTANT: Do not commit your bot token. Put values in Render environment variables or a .env locally.
...
# Default owner ID (change via env if you want)
try:
    OWNER_ID = int(os.getenv("OWNER_ID", "1319292111325106296").strip())
except Exception:
    OWNER_ID = 1319292111325106296

# Extra default admins you asked for (they will always be admins)
...
def exact_word_present(text: str, word: str) -> bool:
    # safe exact word matching (word boundaries)
...
async def send_log_embed(guild: Optional[discord.Guild], title: str, description: str):
    ...
    if not ch and guild:
        try:
            ...
            ch = None
    if ch:
        emb = discord.Embed(title=title, description=description, color=discord.Color.darker_grey(),
...
class SnipeView(discord.ui.View):
    ...
    def make_embed(self) -> discord.Embed:
        ...
        emb.set_author(name=data.get("author_tag", "Unknown"), icon_url=data.get("avatar_url", discord.Embed.Empty))
        emb.set_footer(text=f"{self.idx+1}/{len(self.items)} • {data.get('time','')}")
        if "content" in data:
...
# -------------------------
# SCHEDULED TASK: runs every minute (daily/hourly cats + temp role expiry)
# -------------------------
...
# -------------------------
# on_ready: set streaming presence & start tasks
# -------------------------
@bot.event
async def on_ready():
    # Set streaming presence (purple). Change name/url to your own values.
    try:
        activity = discord.Streaming(name="Max Verstappen", url="https://twitch.tv/yourchannel")
        await bot.change_presence(status=discord.Status.online, activity=activity)
    except Exception:
        # fallback to plain activity
        try:
    ...
    print(f"✅ Bot ready: {bot.user} (id:{bot.user.id}) — guilds: {len(bot.guilds)}")
    add_log("system", f"Bot ready: {bot.user} — guilds={len(bot.guilds)}")


# -------------------------
# Events: message handling & logs
# -------------------------
@bot.event
async def on_message(message: discord.Message):
    ...
    # blocked words detection (compact check)
...
# -------------------------
# Commands (hybrid / support both slash & prefix)
# -------------------------
# Uptime
...
# Servers (admins)
@bot.hybrid_command(name="servers", with_app_command=True, description="List servers the bot is in (admin only)")
...
# Ask for command (ping owner + DM)
@bot.hybrid_command(name="askforcommand", with_app_command=True, description="Ask the owner for a command (pings owner & DMs)")
    ...
    if log_ch:
        ...
        if ch:
            try:
                ...
                pass
    await ctx.reply("✅ Sent your request to the owner (and pinged in logs if configured).", mention_author=False)
    add_log("ask", f"{ctx.author} asked: {request[:200]}")


# Refresh (sync commands)
...
async def refresh(ctx: commands.Context):
    ...
    try:
        ...
        await ctx.reply(f"Failed to refresh: {e}", mention_author=False)


# Restart (admin) - uses Render API if configured
@bot.hybrid_command(name="restart", with_app_command=True, description="Restart the bot (admin). Triggers Render deploy if configured.")
async def restart(ctx: commands.Context):
    ...
    if RENDER_API_KEY and RENDER_SERVICE_ID:
        ...
        await ctx.send(f"Result: {res}") if hasattr(ctx, "send") else None
...
# Eval (owner only)
@bot.hybrid_command(name="eval", with_app_command=True, description="Eval python code (owner only)")
...
# -------------------------
# Admin / pookie / blacklist commands
# -------------------------
...
@bot.hybrid_command(name="addpookie", with_app_command=True, description="Add pookie user (admin)")
...
@bot.hybrid_command(name="removepookie", with_app_command=True, description="Remove pookie user (admin)")
...
@bot.hybrid_command(name="listpookie", with_app_command=True, description="List pookie users (admin)")
async def listpookie(ctx: commands.Context):
    if not is_admin_user(ctx.author.id):
        return await ctx.reply("Admins only.", mention_author=False)
    d = reload_data()
    mentions = [f"<@{uid}>" for uid in d.get("pookies", [])]
    # ping pookies as requested
    await ctx.reply("Pookies:\n" + ("\n".join(mentions) or "None"), mention_author=False)


@bot.hybrid_command(name="blacklist", with_app_command=True, description="Add user to blacklist (admin)")
...
@bot.hybrid_command(name="unblacklist", with_app_command=True, description="Remove user from blacklist (admin)")
async def unblacklist(ctx: commands.Context, user: discord.User):
    if not is_admin_user(ctx.author.id):
        return await ctx.reply("Admins only.", mention_author=False)
    d = reload_data()
...
# -------------------------
@bot.hybrid_command(name="blocked_add", with_app_command=True, description="Add a blocked word (admin)")
...
@bot.hybrid_command(name="blocked_remove", with_app_command=True, description="Remove a blocked word (admin)")
...
@bot.hybrid_command(name="trigger_remove", with_app_command=True, description="Remove a trigger (admin)")
...
async def cmd_afk(ctx: commands.Context, *, reason: Optional[str] = "AFK"):
    d = reload_data()
    d.setdefault("afk", {})
...
@bot.hybrid_command(name="afk_clear", with_app_command=True, description="Clear your AFK")
async def cmd_afk_clear(ctx: commands.Context):
    d = reload_data()
...
# -------------------------
# CAT set channels & command
# -------------------------
@bot.hybrid_command(name="setcatchannel", with_app_command=True, description="Set daily cat channel (11:00 local) (admin)")
async def setcat(ctx: commands.Context, channel: discord.TextChannel):
    if not is_admin_user(ctx.author.id):
        return await ctx.reply("Admins only.", mention_author=False)
    d = reload_data()
...
async def sethourlycat(ctx: commands.Context, channel: discord.TextChannel):
    if not is_admin_user(ctx.author.id):
        return await ctx.reply("Admins only.", mention_author=False)
    d = reload_data()
...
async def cmd_say(ctx: commands.Context, *, text: str):
    ...
    # Confirm to user then send actual message (no pings)
    try:
...
# -------------------------
# Moderation: ban/kick/purge (prefix and slash friendly)
# -------------------------
# Slash versions (require selecting member)
@bot.tree.command(name="ban", description="Ban a member (admin). Slash requires selecting a member.")
...
# Kick
@bot.tree.command(name="kick", description="Kick a member (admin). Slash requires member selection.")
...
# -------------------------
# Roles: give/remove/temp role, mute/unmute, lock/unlock
# -------------------------
...
# -------------------------
@bot.hybrid_command(name="snipe", with_app_command=True, description="Show recent deleted messages in this channel")
...
@bot.hybrid_command(name="esnipe", with_app_command=True, description="Show recent edited messages in this channel")
async def esnipe(ctx: commands.Context):
    items = ESNIPES.get(ctx.channel.id, [])
    if not items:
        return await ctx.reply("No edits to snipe here.", mention_author=False)
    view = SnipeView(items)
    await ctx.reply(embed=view.make_embed(), view=view, mention_author=False)
...
class ShowCommandsView(discord.ui.View):
    ...
    async def select_cb(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        filtered = []
        for c in items:
            # hide admin-only commands for normal users
            ...
            if admin_only and not is_admin_user(interaction.user.id):
                continue
            filtered.append(c)
...
async def showcommands(ctx: commands.Context):
    ...
    if isinstance(ctx, discord.Interaction):
        ...
        await ctx.reply("Pick a category (also sent to your DMs):", mention_author=False)
        try:
...
# -------------------------
# Fun commands: 8ball/jokes/coin/dice/rps/avatar/userinfo/guildinfo
# -------------------------
...
@bot.hybrid_command(name="rps", with_app_command=True, description="Rock Paper Scissors: use rock/paper/scissors")
...
# -------------------------
# Final startup / ensure default admins include owner + extras
# -------------------------
_d = reload_data()
