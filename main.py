...
CAT_API_KEY = os.getenv("CAT_API_KEY", "")  # optional, improves cat images
...
admins: Dict[str, bool] = load_json("admins", {})         # { "id": true }
pookies: Dict[str, bool] = load_json("pookies", {})       # pookie users
blacklist: Dict[str, bool] = load_json("blacklist", {})   # blacklisted user ids
blocked_words: List[str] = load_json("blocked_words", []) # simple substrings
triggers: Dict[str, str] = load_json("triggers", {})      # exact-word -> reply
logs: List[dict] = load_json("logs", [])                  # chronological log entries
...
async def ban_cmd(ctx: commands.Context, user: discord.User, *, reason: str = "No reason"):
...
async def kick_cmd(ctx: commands.Context, user: discord.User, *, reason: str = "No reason"):
    if not is_admin_user(ctx.author):
        return await ctx.send("No permission.")
...
async def blacklist_cmd(ctx: commands.Context, user: discord.User):
    if not is_admin_user(ctx.author):
        return await ctx.send("No permission.")
...
async def unblacklist_cmd(ctx: commands.Context, user: discord.User):
    if not is_admin_user(ctx.author):
        return await ctx.send("No permission.")
...
# Set / remove log channel & cat channel
@bot.command(name="setlogchannel")
async def setlogchannel_cmd(ctx: commands.Context, channel: discord.TextChannel):
...
async def remove_log_channel_cmd(ctx: commands.Context):
...
async def rolldice_cmd(ctx: commands.Context, sides: int = 6):
...
# Trigger management - prefix
@bot.command(name="trigger")
async def trigger_cmd(ctx: commands.Context, action: str, word: Optional[str] = None, *, reply: Optional[str] = None):
    act = action.lower()
    if act == "list":
        if not is_admin_user(ctx.author):
            return await ctx.send("No permission.")
...
# showcommands prefix - only show commands user can use
@bot.command(name="showcommands")
async def showcommands_cmd(ctx: commands.Context):
    # Build a list of visible commands based on admin check
    ...
    admin_only = {
        ...
        "purge","setlogchannel","remove_log_channel","setcatchannel","trigger"
    }
    visible = [c for c in all_cmds if (is_admin_user(ctx.author) or c not in admin_only)]
    await ctx.send("Commands: " + ", ".join(visible))

# Logs prefix: show last N logs
@bot.command(name="logs")
async def logs_cmd(ctx: commands.Context, amount: int = 10):
    if not is_admin_user(ctx.author):
        return await ctx.send("No permission.")
    ...
    # show in an embed - list newest first
    ...
    for i, entry in enumerate(reversed(last), 1):
        ...
        embed.add_field(name=f"{i}. {kind} @ {time}", value=str(detail), inline=False)
...
# Trigger slash group (action: add/remove/list)
...
@tree.command(name="setlogchannel", description="Set logs channel (admin only)")
@slash_admin_like()
...
# showcommands slash - shows only commands user can use
@tree.command(name="showcommands", description="Show commands you can use")
@slash_not_blacklisted()
async def sc_showcommands(inter: discord.Interaction):
    # static mapping of admin-only names (same as prefix)
    admin_only = {
        "ban","kick","blacklist","unblacklist","add_admin","remove_admin","add_pookie","remove_pookie",
        "purge","setlogchannel","remove_log_channel","setcatchannel","trigger","logs"
    }
    # gather slash names registered (use tree commands and prefix commands)
    visible = []
    # check slash commands registered on this tree
    for cmd in tree.get_commands():
        name = cmd.name
        if is_admin_user(inter.user) or name not in admin_only:
            visible.append(name)
    # also include prefix commands (bot.commands)
    for c in bot.commands:
        name = c.name
        if is_admin_user(inter.user) or name not in admin_only:
            visible.append(name)
    visible = sorted(set(visible))
    await inter.response.send_message("Available: " + ", ".join(visible), ephemeral=True)
